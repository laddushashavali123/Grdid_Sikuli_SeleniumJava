...
public class TC28_EUP_Services_Edit_Network_Call_Waiting_To_Disable extends DriverFactory {
	public static final Logger logger = LogManager.getLogger("TC28");
	@Test
	@Parameters({"waitTime","EUP_url","EUP_user","EUP_password", "EUP_domain"})
	public void TC28_EUP_Services_Edit_Network_Call_Waiting_To_Disable_(int timeout, String url, String user, String password, String domain) throws Exception {
		...
		String networkCallWaiting = "Disabled";
		...
		if ( currentNetworkCallWaiting.equals("Disabled") ) {
			logger.info("Set 'Network Call Waiting' as 'Enabled' before running Test case 'Edit ID network call waiting to disabled'");
			serviceT.selectNetworkCallWaiting("Enabled");
		...
		if (result) {
			logger.info("--> Test case 'Edit ID network call waiting to disable': PASSED");
			Reporter.log("--> Test case 'Edit ID network call waiting to disable': PASSED");
		} 
		else {
			logger.info("--> Test case 'Edit ID network call waiting to disable': FAILED");
			Reporter.log("--> Test case 'Edit ID network call waiting to disable': FAILED");
		}
